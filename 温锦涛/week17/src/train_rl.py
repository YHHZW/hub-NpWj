"""
GRPO 训练：Qwen2-0.5B-Instruct 数学题（复合奖励 = 正确分 1.0 + 格式分 0.2）。

与示例 train_grpo.py 的差异：
  - 复用自研 math_problems 模块的 make_question / parse_output / SYSTEM_PROMPT；
  - 训练集难度配比依据基线 probe 的 loose_informative_group_rate 选择（P2/P3/P5）；
  - 复合奖励：reward_correct（宽松解析）+ reward_format（格式，权重 0.2）。

GRPO 核心机制：
  - 无价值网络、无奖励模型：组内 K 条样本的奖励均值/标准差归一化出 advantage；
  - beta=0 时不加载参考模型（TRL 默认），省显存；
  - 每个 prompt 采样 8 条，组内有对有错才有非零 advantage。

使用方式：
  python src/train_rl.py                    # 完整训练（默认 200 步）
  python src/train_rl.py --max_steps 3      # 冒烟测试：验证显存与流程
  python src/train_rl.py --lora             # 全量跑不动时降级为 LoRA
...
from math_problems import SYSTEM_PROMPT, make_question, parse_output

ROOT = Path(__file__).parent.parent
MODEL_PATH = Path(r"D:\badou\八斗课程\pretrain_models\Qwen2-0.5B-Instruct")
OUT_DIR = ROOT / "outputs"

# 训练集难度配比：依据基线 probe 的 loose_informative_group_rate 选择
# P2/P3/P5 为中等难度（既有梯度又有进步空间），P1/P4/P6 不进训练集，留作泛化评估
LEVEL_MIX = [
    ("P3_bracket", 0.50),
    ("P5_unknown_step", 0.25),
    ("P2_two_step", 0.25),
]
    ...
    for _ in range(n):
        ...
        for lv, p in LEVEL_MIX:
            ...
            if r <= acc:
                level = lv
                break
        expr, ans = make_question(level, rng)
        rows.append(
            {
                "prompt": [
...
def main():
    ...
    config = GRPOConfig(
        ...
        # 0/0=NaN，一步训废整个模型。必须显式 bfloat16。
        ...
        # generate 输出完全损坏，GRPO 必须关闭它；0.5B 模型激活值不大，
        # 8GB 显存不开 checkpointing 也够
